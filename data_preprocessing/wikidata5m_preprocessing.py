...
human_entities = wikidata5m_raw_triples['head_entity'][(wikidata5m_raw_triples['relation'] == 'P31') & (wikidata5m_raw_triples['tail_entity'] == 'Q5')]
...
human_entities.to_pickle(os.path.join(BASE_PATH_HOST, 'data_preprocessing/wikidata5m_human_entities.pkl'))

# %% compare my human entities with those from Keidar repo

Keidar_entities, Keidar_relations = pd.read_pickle(os.path.join(BASE_PATH_HOST, 'code_from_other_papers/Keidar_automatic_bias_detec/data/wiki5m/human_ent_rel_sorted_list.pkl'))
Keidar_entities = pd.Series(Keidar_entities)

my_human_ent = pd.read_pickle(os.path.join(BASE_PATH_HOST, 'data_preprocessing/wikidata5m_human_entities.pkl'))
...
# try np.where() - does not work
import numpy as np
np.where(wikidata5m_raw_triples['head_entity'].values == human_entities.values)
...
human_triples_subset.info()

human_triples_subset.to_pickle(os.path.join(BASE_PATH_HOST, 'data_preprocessing/wikidata5m_human_subset_pddf.pkl'))
...
relation_counts[relation_counts >= 10000].plot.bar(title = 'Relations of human entities that have \n at least 10,000 occurrences in Wikidata5M')
