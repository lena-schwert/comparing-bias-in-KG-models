    ...
    for model in KGE_MODEL_NAME_S:
        pipeline_result = pipeline(  # automatic_memory_optimization = True,
            ...
            result_tracker_kwargs = dict(
                ...
                path = os.path.join(BASE_PATH_HOST, 'results', EXPERIMENT_NAME, f'logs_{EXPERIMENT_NAME}_{START_TIME}.csv'),
...
if __name__ == '__main__':
    ...
    print(f'Started Experiment at: {START_TIME}')
    run_pykeen_hpo_pipeline(DEBUGGING = False, KGE_MODEL_NAME_S = ['TransE'],
                            EXPERIMENT_NAME = 'TransE_fullW5M_80epochs', NUM_EPOCHS = 80)
    END_TIME = datetime.now().strftime("%d.%m.%Y %H:%M")
    print(f'Finished Experiment at: {END_TIME}')
