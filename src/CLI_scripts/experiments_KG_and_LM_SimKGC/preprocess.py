...
def _load_wiki5m_id2text(path: str, max_len: int = 50):
    ...
    for line in open(path, 'r', encoding='utf-8'):
        fs = line.strip().split('\t')
        assert len(fs) == 2, 'Invalid line: {}'.format(line.strip())
        assert fs[0].startswith('Q'), 'Invalid line: {}'.format(line.strip())
...
def preprocess_wiki5m(path: str, is_train: bool) -> List[dict]:
    if not wiki5m_id2rel:
        _load_wiki5m_id2rel(path='{}/relation2label_wikidata5m_human_08042022_v2.tsv'.format(os.path.dirname(path)))
