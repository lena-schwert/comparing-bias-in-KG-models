...
class PredInfo:
    ...
    pred_score: float
    topk_scores: list
    topk_scores_ids: list
    topk_scores_labels: list
    rank: int
    correct: bool
...
def eval_single_direction(predictor: BertPredictor,
    ...
    for idx, ex in enumerate(examples):
        ...
        # this retrieves the internal numeric IDs for the top k entities
        cur_topk_indices = topk_indices[idx]
        pred_idx = cur_topk_indices[0]
        # original code: cur_score_info = {entity_dict.get_entity_by_idx(topk_idx).entity: round(topk_score, 3)
        #                   for topk_score, topk_idx in zip(cur_topk_scores, cur_topk_indices)}
        # this accesses the shorter labels based on the numeric IDS
        cur_score_labels = [entity_dict.get_entity_by_idx(topk_idx).entity
                            for topk_idx in cur_topk_indices]
        # this accesses the Wikidata Q-ID based on the numeric IDS
        cur_score_QIDs = [entity_dict.get_entity_by_idx(topk_idx).entity_id for topk_idx in
                            cur_topk_indices]

        pred_info = PredInfo(head_id = ex.head_id, head=ex.head,
                             relation_id = ex.relation_id, relation=ex.relation,
                             tail_id = ex.tail_id, tail=ex.tail,
                             pred_tail_id = entity_dict.get_entity_by_idx(pred_idx).entity_id,
                             pred_tail=entity_dict.get_entity_by_idx(pred_idx).entity,
                             pred_score=round(cur_topk_scores[0], 4),
                             #topk_score_info=json.dumps(cur_score_info),
                             topk_scores=cur_topk_scores,
                             topk_scores_ids = cur_score_QIDs,
                             topk_scores_labels=cur_score_labels,
                             rank=ranks[idx],
    ...
    logger.info('Evaluation took {} seconds'.format(round(time() - start_time, 3)))
    return metrics
