...
from transformers import AdamW, get_linear_schedule_with_warmup  # instead of BertAdam, WarmupLinearSchedule
...
class KGProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        return self._create_examples(
            # set type "train" here for the _create_examples function
                ...
                if rnd <= 0.5:
                    ...
                    for j in range(5):
                        ...
                        while True:
                            ...
                            tmp_tail = random.choice(tmp_ent_list)  # choose a random entity from list
                            # create new TSV-separated triple using new tail entity
                            tmp_triple_str = line[0] + '\t' + line[1] + '\t' + tmp_tail
                            if tmp_triple_str not in lines_str_set:
                                break
                        tmp_tail_text = ent2text[tmp_tail] # retrieve text version of the corrupt entity
                        # append corrupt/negative example to list of InputExample objects with label 0 = False
                        examples.append(
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, print_info = True):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if example.text_b and example.text_c:
            ...
            # TODO shorten the whole triple token sequence if necessary to comply with max sequence length
        ...
        #
        # TODO This concept corresponds to segment embeddings described in the paper
        ...
        assert len(input_ids) == max_seq_length, "Input ID list must have same length as max sequence length."
        assert len(input_mask) == max_seq_length, "Input mask list must have same length as max sequence length."
        assert len(segment_ids) == max_seq_length, "Segment ID list must have same length as max sequence length."
...
def main():
    ...
    logging.basicConfig(format = '%(asctime)s - %(levelname)s - %(name)s -   %(message)s',
                        ...
                        # TODO add arguments for logging to file
                        #filename = 'umls_log_debugging.txt',
                        #filemode = 'w')  # options: 'a', 'w'
