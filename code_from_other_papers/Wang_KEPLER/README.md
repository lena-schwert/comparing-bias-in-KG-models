...
Refer to [the RoBERTa document](https://github.com/THU-KEG/KEPLER/blob/main/examples/roberta/README.pretraining.md) for the detailed data preprocessing of the datasets used in the Masked Language Modeling (MLM) objective.

### 

### Preprocessing for KE data

The pre-training with KE objective requires the [Wikidata5M dataset](https://deepgraphlearning.github.io/project/wikidata5m). Here we use the transductive split of Wikidata5M to demonstrate how to preprocess the KE data. The scripts used below are in [this folder](https://github.com/THU-KEG/KEPLER/blob/main/examples/KEPLER/Pretrain). 
...
**Note:** If you are interested in the detailed implementations. The main implementations are in [tasks/MLMetKE.py](https://github.com/THU-KEG/KEPLER/blob/main/fairseq/tasks/MLMetKE.py) and [criterions/MLMetKE.py](https://github.com/THU-KEG/KEPLER/blob/main/fairseq/criterions/MLMetKE.py). We encourage to master the fairseq toolkit before learning KEPLER implementation details.
...
To directly fine-tune KEPLER on TACRED in fairseq framework, please refer to [this script](https://github.com/THU-KEG/KEPLER/blob/main/examples/KEPLER/TACRED/TACRED.sh). The script requires 2x16GB V100 GPUs.
...
Please refer to [this directory](https://github.com/THU-KEG/KEPLER/blob/main/examples/KEPLER/OpenEntity) and [this script](https://github.com/THU-KEG/KEPLER/blob/main/examples/KEPLER/OpenEntity/run_openentity.sh) for the codes of OpenEntity experiments.
...
For the fine-tuning on GLUE tasks, refer to the [official guide of RoBERTa](https://github.com/THU-KEG/KEPLER/blob/main/examples/roberta/README.glue.md).

Refer to [this directory](https://github.com/THU-KEG/KEPLER/blob/main/examples/KEPLER/GLUE) for the example scripts along with hyper-parameters.
...
First, install the `graphvite` package in[`./graphvite`](https://github.com/THU-KEG/KEPLER/blob/main/graphvite) following its instructions. [GraphVite](https://github.com/DeepGraphLearning/graphvite) is an fast toolkit for network embedding and knowledge embedding, and we made some modifications on top of them.

Generate the entity embeddings and relation embeddings with[`generate_embeddings.py`](https://github.com/THU-KEG/KEPLER/blob/main/examples/KEPLER/KE/generate_embeddings.py). The arguments are as following:

- `--data`: the entity decription data, a single file, each line is an entity description. It should be BPE encoded and binarized  like introduced in the [**Preprocessing for KE data**](https://github.com/THU-KEG/KEPLER/blob/main/README.md#KEpre)
...
Then use [`evaluate_transe_transductive.py`](https://github.com/THU-KEG/KEPLER/blob/main/examples/KEPLER/KE/evaluate_transe_transductive.py) and [ `ke_tool/evaluate_transe_inductive.py`](https://github.com/THU-KEG/KEPLER/blob/main/examples/KEPLER/KE/evaluate_transe_inductive.py) for KE evaluation. The arguments are as following:
